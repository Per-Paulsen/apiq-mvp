...
1. **Should we now do verbatim-population (T-Verbatim-Population) as separate Welle?** Currently 0/213 sources have `quote`. This means T25 verifies nothing. If we want T25 to have real value (catch RFC-URL-rewrites in quarterly runs), need ~30-50 high-value rules with manually-curated RFC-quotes.
...
1. **Should per-test timeouts in run-deterministic-layer.test.ts be normalized across all reference-specs?** Currently dnd5eapi=90s, stripe-full=30min, others use defaults. Stripe-full's 24-min run feels excessive for CI.
...
i dont know? but what is the right thing to do? and then do it now!

1. **Are 91 custom-functions sustainable / will they hit Spectral's loading-overhead?** Spectral lazy-loads, so cold-start unaffected. But each `spectral.run()` invokes potentially-many custom-functions per spec. Phase-3-integrator flagged this as Phase-B-perf-investigation territory.
...
## Open Questions — Architectural concerns (post-results-doc, raised 2026-05-09)

> Diese Punkte gehen über die 4 Welle-D-spezifischen OQ hinaus. User-Frage 2026-05-09 nach Welle-D-Commit: "machen wir das gerade überhaupt professionell bzw optimiert genug oder mit der Brechstange?". Ehrliche Selbst-Einschätzung war: mostly right, aber spezifische tech-debt-Punkte. Diese 4 Punkte sind nicht in Plan-Doc Welle-Arch (das ist nur file-tree-Refactor) — sind eigene strategische Entscheidungen.

### A1 — Three-source-of-truth-Problem (patterns.json + apiq-meta-blocks + rules-brainstorm.md)

Dieselbe Pattern-Info lebt in 3 places:

- `scripts/spike/data/patterns.json` (Mining-Output, 959 entries, Zod-schema-validated NICHT — flat array)
- `apiq-meta`-blocks in 11 yamls (per-rule, F5-coverage-gate validates fields-exist aber nicht type-shape)
- `rules-brainstorm.md` (Discussion + Rationale, ~2327 Zeilen)

Risiko: drift across sources. Z.B. wenn ein patterns.json-pattern severity-Hypothesis-changed wird, müsste auch yaml + brainstorm-doc aktualisiert werden — aktuell manuell + ohne checksum.

**Lösungs-Optionen:**

1. `patterns.json` ist canonical; yamls + brainstorm sind generated-from. Build-script `npx tsx scripts/spike/data/regenerate-rules.ts` regeneriert.
2. apiq-meta-blocks sind canonical; patterns.json wird von ihnen abgeleitet. Brainstorm-doc bleibt diskussions-territory ohne authority.
3. Tooling: lint-check der drift detected zwischen den 3 sources; CI-fail bei drift.

**Meine Recommendation:** Option 3 ist least-disruptive (keine breaking-changes für existing consumers von patterns.json). Build aus apiq-meta-blocks ist semantisch sauberer aber breaking.

→ User-Decision:

---

### A2 — Loose typing on patterns.json + apiq-meta-blocks

`patterns.json` lädt aktuell als plain JSON. Keine Zod-validation. Wenn ein Mining-Subagent versehentlich einen typo'd `lens`-value einfügt, passiert nichts beim Load.

apiq-meta-blocks haben F5-coverage-gate (alle Pflichtfelder da?) aber nicht type-validation (sind die Werte well-shaped?).

**Lösungs-Optionen:**

1. `PatternSchema` (Zod) für patterns.json; load-time-validation in Pattern-Knowledge-Index + im Test-Suite.
2. Per-rule apiq-meta-Block Zod-validation; z.B. `iso25010` muss array-of-known-values sein, `lenses` muss aus closed-set.
3. Beides.

**Meine Recommendation:** Option 3. Both. Should be ~1-2 Subagent-tasks combined. Würde die three-source-of-truth (A1) stützen weil mit strict types drift expliziter wird.

→ User-Decision:

---

### A3 — Custom-function Registry / Schema (für die 91 functions)

8 function-files, 91 functions total. Jede function hat eine signature (`(targetVal, options, context) => string[]|null`) und ist registriert in `APIQ_CUSTOM_FUNCTIONS` map + `SUPPORTED_FUNCTIONS` set.

Aber: **kein Schema** für was eine function exposen sollte:

- Eingabe-shape (was ist `targetVal`? was sind valid `options`?)
- Output-shape (was returnen sie? Findings? Booleans? RuleResults?)
- Metadata (welcher lens? welcher pattern-id? welche performance-class O(n) oder O(n²)?)

Bei 91 functions wird das schwer zu warten. Bei 200+ unwartbar.

**Lösungs-Optionen:**

1. `FunctionMetadata`-Type: jede function exportiert neben dem callable auch ein Metadata-Object (lens, pattern-id, perf-class, options-schema).
2. Pure docstring-convention (JSDoc) + lint-rule die enforced.
3. Beide: typed Metadata + JSDoc für freeform-context.

**Meine Recommendation:** Option 1 (typed Metadata) als Welle-Arch-Erweiterung. Würde mit File-Tree-Refactor zusammen passen (functions ziehen mit ohne ihre Metadata zu verlieren). Option 2 alleine ist schwächer weil nicht runtime-checkable.

→ User-Decision:

---

### A4 — Already-logged tech-debt-Tasks (re-listing for completeness)

Diese sind bereits in Plan-Doc / Memory getrackt, aber zur Vollständigkeit hier:

- **T-Stripe-Perf** (Welle-E §9): profile + optimize 23.7-min stripe-full-test. Suspect O(n²) custom-functions, repeated JSONPath-compilation, lack-of-rule-batching.
- **T-Verbatim-Population** (Welle-E oder Welle-Doc): manuelle Kuration ~30-50 high-value rules mit echten RFC-quotes damit T25 Wert hat.
- **F7-bulk-pass narrowing** (refinement-task): 7-language-SDK-list ist für viele rules zu wide. Per-rule narrowing wo applicable.

**Meine Recommendation:** alle 3 bleiben in ihren bisherigen Wellen-Slots. Keine Aktion hier.
