...
1. `**affectedEndpoints` in aggregierten Findings ist deduped union — könnte bei einem Pattern auf 1145 endpoints (github-rest) groß werden (max ~1145 entries).** Phase-B-Token-Budget muss das berücksichtigen — eine codegen-aggregated finding könnte ~50KB JSON sein wenn alle 1145 endpoints in `affectedEndpoints` stehen.
...
müssen wir das jetzt schon berücksichtigen oder erst wenn phase b dran ist?

1. **STAGE-A-RESULTS.md regeneration-Zyklus.** Q2-verify hat die Datei mit nur dnd5eapi überschrieben. Welle V V2 wird final regenerieren. Aber: zwischen jetzt und Welle V werden weitere Wellen (M / F / C / D / D2 / E / R) andere Tests laufen lassen die via `stage-a-validation.ts` die Datei regenerieren würden — jedes Mal neu/unvollständig.
...
lass uns das auch jetzt machen oder?
...
## Open Questions — User Review + Resolutions (2026-05-06 evening)

User hat alle 4 Open Questions im Doc beantwortet + zusätzlich die Memory-Regel "**niemals Fixes auf später verschieben**" klargestellt (`memory/feedback_never_defer_fixes.md`). Per dieser Regel sind alle Verschiebungs-Recommendations zu re-evaluieren — kein Fix der konkret + fixable ist darf in zukünftige Welle aufgeschoben werden.

### Q1 (`aggregateCodegen` default-Policy)
**User:** "ok" → accept Recommendation.
**Resolution:** current default `true` belassen. Falls in Phase-B-Engineering Cross-Linter-Compare zeigt dass kleine Specs durch Aggregation Information verlieren, dort thresholdshohe-config umstellen.

### Q2 (`affectedEndpoints` cap bei aggregierten Findings)
**User:** "müssen wir das jetzt schon berücksichtigen oder erst wenn phase b dran ist?"
**Initial Recommendation:** "in Welle F (F4)" — REVIDIERT per Memory-Regel "niemals Fixes verschieben".
**Resolution:** **JETZT als Q8 follow-up.** Cap auf max 100 entries pro aggregated finding + `meta.aggregateAffectedEndpointsTotal: <originalCount>` als zusätzliches Meta-Field. Phase-B-Cleanup-Layer-3 Per-Endpoint-Slicing kann die vollständige Liste aus den raw findings rekonstruieren wenn nötig (durch `aggregateCodegen: false` opt-out).

### Q3 (STAGE-A-RESULTS.md regeneration-Zyklus)
**User:** "wieso können wir das nicht schon jetzt machen?"
**Resolution:** **JETZT als Q6 follow-up.** Refactoring von `stage-a-validation.ts` so dass `--specs=X,Y,Z` argument NICHT die ganze Datei überschreibt sondern nur die genannten Sections updated (existing Sections für nicht-spezifizierte Specs bleiben). Plus optional `--write-output=<path>` flag für isolated runs (z.B. `/tmp/stage-a-experimental.md`). Tests dafür.

### Q4 (Q3 retagging-Approach robust)
**User:** "lass uns das auch jetzt machen oder?"
**Resolution:** **JETZT als Q7 follow-up.** `ALL_MODULES`-entries in `modules/index.ts` bekommen explizites `layer: 'module-class'` field. Post-collection retag-loop ersetzt durch `f.layer = mod.layer` (mit Default-Fallback falls Modul-internes layer-tag explicit ist). Zukünftige Module mit anderem Layer können das via override setzen.

---

## Q6 / Q7 / Q8 Follow-up Implementation

Sofort nach Q1-Q5 implementiert (gleiche Welle-Q-Session 2026-05-06 evening), per Memory-Regel "niemals Fixes verschieben".

### Q6 — STAGE-A-RESULTS.md Regeneration-Zyklus Fix

- **Refactoring:** `scripts/spike/eval/stage-a-validation.ts` Markdown-Render-Pfad so dass bei `--specs=X,Y,Z` argument nur die Sections für X/Y/Z im md-File updated werden; Sections für nicht-spezifizierte Specs bleiben unverändert.
- **Plus optional `--write-output=<path>` flag** für isolated runs ohne Side-Effect auf canonical STAGE-A-RESULTS.md.
- **Tests:** unit-test für die Section-Merge-Logik.

### Q7 — Q3 retagging robustify

- **`scripts/spike/deterministic/modules/index.ts` ALL_MODULES** entries erweitert um `layer: DetectorLayer` field (default `'module-class'`).
- **`runModules` retag-loop** ersetzt durch `f.layer = mod.layer ?? 'module-class'` (Fallback für Modul-Findings die explizit anderen Layer tagen sollen).
- **Tests:** integration-test bleibt grün.

### Q8 — affectedEndpoints cap

- **`scripts/spike/deterministic/output-mapper.ts` `aggregateCodegenFindings`:** affectedEndpoints cap auf max 100 entries (deduped union von raw findings, dann sliced).
- **`meta.aggregateAffectedEndpointsTotal`** als zusätzliches Meta-Field (full count, vor cap).
- **Tests:** erweiterte test-cases für cap-behavior.

### Tests post Q6+Q7+Q8

`cd scripts/spike && npx vitest run` → erwartete count: 802 + neue Q6/Q8-tests (Q7 ist refactor-only, nutzt existing tests). Genaue Numbers in commit-message.

### Verification

- 4-Spec stage-a-validation re-run nach allen Fixes → STAGE-A-RESULTS.md komplett mit allen 4 Specs + Embedding-Coverage (per Q2 Fix erstmals verfügbar).
- Embedding-Coverage post-fix:
  - stripe-full: 62.1% Jaccard / 62.1% Embedding (no-op weil reference-titles = stage-a titles)
  - pagerduty-full: 30.4% / **69.6% Embedding** (+39.2pp)
  - dnd5eapi: 35.7% / **85.7% Embedding** (matches prediction!)
  - github-rest: 25.8% / **64.5% Embedding** (+38.7pp)
- **Strategic implication:** Delta vs Predicted shrinkt von −50 bis −61pp (Jaccard) auf −20 bis −22pp (Embedding-best). Vocabulary-Drift war der dominant Gap. Plan-Doc Welle C/D/E "value-add nicht load-bearing"-Framing wird daraufhin re-evaluiert post-commit.
