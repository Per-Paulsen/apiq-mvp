...
- **Prisma type imports** (per Epic 04 results §6): `Finding`, `Spec`, `SpecVersion` aliases live at `@/generated/prisma/client` (NOT `@/generated/prisma/models` despite CLAUDE.md). Use `@/generated/prisma/client` for all type imports in this epic — see Epic 04's `src/lib/analysis/quality-score.ts` and `src/lib/analysis/runAnalysis.ts` for the established pattern.
...
- **Schema addition: `Finding.staleReason: String?`** (per cross-epic Q2.1, 2026-05-02). Rationale: when `validatePatchOps` returns `!applyClean` OR `hallucinationCheck.hallucinated`, the validator already produces a verbatim diagnostic (`applyError` for `fast-json-patch.validate` failures, `hallucinationCheck.details` for hallucinated paths). Without persisting this string, the user sees only the generic "This patch is no longer applicable" hint with no signal on WHY — for an engineer audience debugging a stale apply, that's insufficient. Add `staleReason: String?` (nullable) to `Finding` via Prisma migration in Epic 06's first scope step. Server-side: `applyFindingAction` step 5 writes the validator's diagnostic to `staleReason` in the same transaction as `status='stale'`. Client-side: `finding-card.tsx`'s stale-card UI renders a small `<details><summary>Why?</summary><pre className="font-mono text-xs">{finding.staleReason}</pre></details>` below the inline "Re-analyze to refresh" hint when `staleReason` is non-empty. Cost: ~15 LOC + one migration. Stale findings are residual (≤6.7% per Epic 00 spike) but non-zero; the diagnostic earns its weight against debug time.
- Server actions:
  - `applyFindingAction({ findingId })`:
    ...
    5. Validate `patchOps` against `Spec.currentJson` using `validatePatchOps` from `src/lib/analysis/validate-patches.ts` (ported from `scripts/spike/validate-patches.ts` — Epic 00 reference implementation). The validator combines:
       ...
       If validation fails (`!applyClean` OR `hallucinated`) → set finding `status = 'stale'` AND `staleReason = <validator diagnostic>` (the existing `applyError` for clean-apply failures, or `hallucinationCheck.details` for hallucinated paths — same string used in the returned `message`) in a single Prisma `update` call, then return `{ kind: 'patch_stale', message }`. **Do not** mutate the spec. The validator MUST be called against the cycle-stripped `currentJson` (Epic 03 stores it that way; Epic 04 prompts the LLM with the same shape — all three observers agree).
    ...
    7. In a single transaction:
       ...
       - **recompute** `Spec.qualityScore = computeQualityScore(allFindingsForSpec)` using the deterministic formula from Epic 04 (`src/lib/analysis/quality-score.ts`). The function accepts the full Finding array and filters `status === 'open'` internally — callers do NOT need to pre-filter. The apply moves one finding from `open` to `applied`, so the score should usually rise. Pure function — no LLM call.
...
- Wire these actions into the Spec Detail screen (Epic 05): enable the Apply / Reject buttons, add Undo Apply on `applied` cards, Undo Reject on `rejected` cards. `stale` and `outdated` cards show a read-only badge with a "Re-analyze to refresh" hint that calls `reanalyzeSpecAction` (Epic 04). **No client-side optimistic state needed** for the Re-analyze button — `reanalyzeSpecAction` writes `Spec.analysisStatus = 'analyzing'` synchronously before returning (per Epic 04 results §"Resolved Q3" / commit `50b4b1c`); the Spec Detail poller picks up the new status on its next 3 s tick. Consumers may also call `router.refresh()` in the click handler to skip the next poll wait.
- When `applyFindingAction` returns `{ kind: 'patch_stale' }`, the UI MUST NOT show a destructive / error toast. Instead it (a) silently re-renders the finding card in its new `stale` state (status badge changes from `open` to `stale`), and (b) shows a non-blocking inline hint on that single card: "This patch is no longer applicable to the current spec. Re-analyze to refresh." with a "Re-analyze" button calling `reanalyzeSpecAction` (Epic 04). When `Finding.staleReason` is non-empty, render an additional collapsible diagnostic below the hint: `<details><summary>Why?</summary><pre className="font-mono text-xs">{finding.staleReason}</pre></details>`. Per Epic 00 results: hallucinated patches are an expected residual (≤6.7% in the spike), and the user should never see an apply-error toast for them — but the engineer audience does need the verbatim validator diagnostic to know whether to investigate or just re-analyze. **Toast wiring** (per Epic 08 §"Toast wiring on existing surfaces"): the stale-card's Re-analyze button — like Epic 05's header Re-analyze button + the failed-card Retry button — calls `showToast(TOASTS.reanalyzeStarted)` from `@/lib/toasts` after `reanalyzeSpecAction({ specId })` returns success, BEFORE `router.refresh()`. Epic 08 owns the wiring location; Epic 06 just emits the action call from the stale-card button.
...
- Add a **Versions drawer** to Spec Detail: collapsible side drawer listing all SpecVersions for the spec (newest first), each row showing `versionNumber`, `label`, `createdAt`. Read-only — no rollback-to-version action in v0.1 (see Out of scope). **Open/closed state must be controlled** via `useState<boolean>(drawerOpen)` (per cross-epic Q1, 2026-05-02). Spec Detail polls every 3 s while `analysisStatus` is `pending`/`analyzing` (Epic 05's `spec-detail-view.tsx` calls `router.refresh()` per tick); uncontrolled `<details>`/`Drawer` collapsibles lose state across the resulting re-renders, slamming the drawer shut every 3 s while a spec analyzes — controlled state preserves it. ~3 LOC overhead.
...
- Tests (Vitest):
  ...
  - **Pure-function tests** at `src/__tests__/llm-pipeline/validate-patches.test.ts` (per cross-epic Q2): assert `validatePatchOps` directly against AC #2's four shapes — `add` whose parent is missing → hallucinated; `replace`/`remove`/`test` with missing path → hallucinated; `move`/`copy` with missing `from` → hallucinated; `move`/`copy` with EXISTING destination `path` → NOT hallucinated (validator must NOT check destination — bug-fix #1 from the spike, must remain green).
...
2. Apply on a finding whose `patchOps` fail validation flips the finding to `status = 'stale'` AND writes the validator's verbatim diagnostic (`applyError` or `hallucinationCheck.details`) to `Finding.staleReason` in the same transaction; does NOT mutate the spec, and creates no SpecVersion; the server action returns `{ kind: 'patch_stale', message }`. The stale-card UI renders a collapsible `<details><summary>Why?</summary><pre>{staleReason}</pre></details>` below the inline "Re-analyze to refresh" hint when `staleReason` is non-empty. The three hallucination shapes that must each be tested:
...
- (resolved per cross-epic Q2, 2026-05-02) `validatePatchOps` test coverage: Epic 06 ships BOTH integration tests (AC #2 / 2a-2d via `applyFindingAction`) AND a separate pure-function test file `src/__tests__/llm-pipeline/validate-patches.test.ts` covering the same four shapes against the pure function directly. The pure-function tests fit Epic 04's library-test style (the file lives under `llm-pipeline/` because that's where the source lives), and the integration tests assert end-to-end behaviour on apply.
...
- (resolved per cross-epic Q1, 2026-05-02) Versions drawer open/closed state: option (a) — controlled `useState<boolean>(drawerOpen)`, preserves state across the 3 s polling re-renders. Documented in Scope §"Versions drawer".
- (resolved per cross-epic Q2.1, 2026-05-02) `stale`-card diagnostic: option (c) — persist the validator's verbatim diagnostic to `Finding.staleReason` and surface it in a collapsible `<details><summary>Why?</summary><pre>{staleReason}</pre></details>` below the inline "Re-analyze to refresh" hint. Engineer-audience needs the diagnostic to debug stale applies; cost is ~15 LOC + one migration. Schema addition documented in Scope §"Schema addition: `Finding.staleReason: String?`"; server-side write in `applyFindingAction` step 5; AC #2 updated.
