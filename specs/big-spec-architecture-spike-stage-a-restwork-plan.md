...
**Pre-Welle-Audit-Standard (post-Welle-I, REPLACES manual grep):** Manueller Audit ist OBSOLET ab Welle I. Stattdessen: konsultiere `scripts/spike/eval/INVENTORY.md` + `COVERAGE.md` + `CROSS-REFERENCES.md` + `DRIFT-REPORT.md` + `TEST-COVERAGE.md` + `API-SURFACE.md`. Welle-Specs zitieren explizit aus diesen Files. Wenn die Files outdated sind: `npm run inventory-all` + commit BEFORE Welle-Spec-writing. Für slow `COVERAGE.md` (~45min, 4-Spec-Run): `npm run build-coverage` separat; auto-refreshed nightly via `coverage-nightly.yml`. CI-gate (`inventory-drift.yml`) failt PRs mit staler INVENTORY/CROSS-REFERENCES/DRIFT-REPORT/TEST-COVERAGE/API-SURFACE. Memory: `feedback_consult_inventory_first.md`.
...
| **I** | `specs/E09-w-i-inventory-capability-map.md` (+ `*-results.md`) | ✓ 2026-05-10 | TBD (regenerated post-Welle-I; see commit) | TBD (this commit) | done; **4 parallele Subagents** (inventory-agent + coverage-agent + cross-refs-agent + wrap-agent); **8 sub-tasks** I1-I8 implementiert: I1 build-inventory.ts (yaml-rules + modules + walkers + classifiers + functions + tests + patterns + apiq-meta-fields scan; outputs `inventory.json` gitignored + `INVENTORY.md` committed) + I2 build-coverage-map.ts (4-spec runtime extraction, ~45min sequenziell; outputs `coverage.json` gitignored + `COVERAGE.md` committed) + I3 Cross-References + Drift-Detection (5 drift-classes: substrate-only / dead-code / orphan-module / function-binding-broken / claimed-vs-actual-mismatch; outputs `CROSS-REFERENCES.md` + `DRIFT-REPORT.md` committed) + I4 Test-Coverage-Map (`TEST-COVERAGE.md` committed; untested-detectors + test-orphans identified) + I5 API-Surface-Export (`API-SURFACE.md` committed; public-vs-internal + unused-exports) + I6 CI-Integration (`inventory-drift.yml` per-PR fast ~3min + `coverage-nightly.yml` cron `0 3 * * *` ~45min auto-PR via peter-evans/create-pull-request@v6; npm-scripts `build-inventory` + `build-coverage` + `cross-refs` + `drift-report` + `test-coverage-map` + `api-surface` + `inventory-all` in package.json) + I7 Plan-Doc + Memory updates (§2 post-Welle-I-Standard added + `feedback_consult_inventory_first.md` memory + MEMORY.md pointer + CLAUDE.md status-block) + I8 Welle-E-Spec ready-to-go (E0+E1 sub-tasks reflektieren Welle-I-Substrate-Nutzung); **6 committed markdown-files** als single-source-of-truth (INVENTORY/COVERAGE/CROSS-REFERENCES/DRIFT-REPORT/TEST-COVERAGE/API-SURFACE); **2 gitignored JSON-files** (inventory.json + coverage.json) regenerable via npm-scripts; **INVENTORY.md ground-truth-totals:** 354 yaml-rules / 127 functions / 26 aggregators / 13 modules / 4 classifiers / 101 test-files / 972 patterns; **DRIFT-REPORT.md initial-baseline:** 585 substrate-only (Stage-B-territory expected) + 0 dead-code (coverage.json pending nightly run) + 2 orphan-module (`spec-diff.ts` known-2-Spec-lifecycle + `style-classifier.ts` library-helper) + 0 function-binding-broken + 56 claimed-vs-actual-mismatches (mostly stale numbers in CLAUDE.md status-block + Plan-Doc-§5/§7/§21-rows: e.g. `342→354 yaml_rules`, `116→127 functions`, `25→26 aggregators`, `15→13 modules` post-Arch+/D2); per "niemals defern": fix-able doc-drifts in Resolution-Pass-Commit nach Welle-I-Main (separate sub-task; baseline drift-counts dokumentiert hier zur Regression-Detection) |
