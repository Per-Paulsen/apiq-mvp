...
- **Toast wiring on existing surfaces** (per Epic 05 results §"Risks for future epics → Epic 08" + Epic 06 §"stale-card Re-analyze"): Epic 05 left several click-handlers calling `router.refresh()` silently; Epic 06's stale-card Re-analyze button shares the same call-site pattern. Epic 08 wires the `TOASTS` catalog into all of them so the user gets visible feedback. Specifically:
