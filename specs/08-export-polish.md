...
- **`formatAnalysisError(raw: string): { headline: string; details?: string; budgetShape?: { spent: number; limit: number; retryAt: string } }`** at `src/lib/format-analysis-error.ts` (per cross-epic Q6 — owned by Epic 08, consumed by Epic 05's failed-card and the Spec-Detail budget-toast hook below). Parsing rules:
  ...
  Vitest unit tests cover all three branches.
- **Sidebar hydration warning fix** (per cross-epic Q5 — pre-existing across Epic 03 / Epic 04 browser verification): `SidebarMenuButton` with the `tooltip` prop emits a `data-state` attribute that mismatches between SSR and client (the controlled-tooltip primitive defaults to a different `data-state` server-side vs after hydration). Investigate the root cause in `(app)/layout.tsx` / shadcn's sidebar primitive; likely fix is one of (a) defer Tooltip mount until after hydration via `useEffect`-gated client component, (b) pin the Tooltip's controlled `open` state to `false` on SSR, or (c) suppress hydration on the offending element with `suppressHydrationWarning`. Pick whichever has the lowest blast radius; document the choice in Epic 08 results. Verify the warning is gone in browser DevTools console after the fix.
...
- **Toast system**: shadcn `Toaster` mounted in `(app)/layout.tsx`, top-right, success / error / info variants per `prd-decisions.md` §"Components" Toasts. Epic 08 ships:
  ...
  - **Canonical message catalog** — extends `src/lib/toasts.ts` (the Epic 03 stub, which already exports `formatQuotaToast` per Epic 03 results) with a `TOASTS` constant. Do NOT re-create the file; add the catalog and `showToast` helper alongside the existing `formatQuotaToast`. Decision per `specs/cross-epic-review.md` Q1 (option a — single source of truth, easy to i18n later, consistent tone). v0.1 catalog (initial entries — emitting epics may add more):
    ```ts
    export const TOASTS = {
...
- Standardise the **quota-exceeded** error shapes across all server actions:
  ...
  Both shapes share the `retryAt` field. **Per-consumer pattern** (per Epic 02 results — there is no global "last action state" subscription in React/Next.js without custom infra; v0.1 keeps it simple): each consuming epic's UI surface (Add Spec form in Epic 03, Spec Detail re-analyze button in Epic 04, Apply / Reject buttons in Epic 06) detects these shapes from its own `useActionState` (or `useTransition`) result and calls `showToast(formatQuotaToast(error))`. **Asynchronous-budget-rejection surfacing**: per Epic 04 results §"Risks for Epic 08", `runAnalysis`'s budget rejection arrives as `Spec.analysisError` (a `'Daily LLM budget reached ($X / $Y) — resets at <ISO>'` string written by runAnalysis), NOT as a synchronous error from `reanalyzeSpecAction`. The Spec Detail screen (Epic 05) detects the budget-shape on its poll-tick: if `Spec.analysisStatus === 'failed'` AND `Spec.analysisError` matches the "Daily LLM budget reached" pattern, the Spec Detail page calls `showToast(formatQuotaToast({ kind: 'budget_exceeded', spent, limit, retryAt }))` once per session per specId (sessionStorage dedupe key: `apiq.budget-toast.<specId>`). The failed-card itself renders the formatted message regardless. Epic 08 ships the formatter helper at `src/lib/toasts.ts`:
  ```ts
...
17. **`formatAnalysisError`** at `src/lib/format-analysis-error.ts` is implemented per the parsing rules in Scope §"Polish". Vitest covers all three branches (budget-shape regex match, zod-error JSON, plain message fallthrough). Both Epic 05's failed-card and the Spec-Detail budget-toast hook (Scope §"Rate-limit polish") import it — no inline parsing duplication.
18. **Sidebar hydration warning fix** (per Scope §"Polish"): browser DevTools console shows zero hydration-mismatch warnings on `(app)` routes after the fix. Verified manually via Playwright on at least one `(app)` page (e.g. `/specs`). Epic 08 results documents which fix path was chosen (a / b / c) and why.
19. **Pre-launch checklist reconciliation** (per cross-epic Q7): every item in CLAUDE.md §"Pre-launch checklist (open issues)" is either resolved (verified by test or manual check) or explicitly deferred to v0.2 with documented reasoning in `specs/08-export-polish-results.md`. Epic 04/05/06/07 results are also scanned for follow-up items not yet in the checklist. The reconciliation is the explicit launch-gate for v0.1; Epic 08 does NOT close until every item has either a `RESOLVED` or `DEFERRED-V0.2` annotation.
