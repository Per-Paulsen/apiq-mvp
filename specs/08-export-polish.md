...
- **Toast wiring on existing surfaces** (per Epic 05 results §"Risks for future epics → Epic 08" + Epic 06 §"stale-card Re-analyze"): Epic 05 and Epic 06 already ship the `showToast` calls at these sites; Epic 08 completes the infrastructure (canonical `TOASTS` catalog + `Toaster` mount + runtime body). The wiring points are:
  ...
  - Specs-list (Epic 07) row-action Re-pull: after `repullSpecAction({ specId })` returns success, call `showToast(TOASTS.rePullComplete)` BEFORE `router.refresh()`. On `error.kind === 'rate_limited'` call `showToast(formatQuotaToast(error))` (re-pulls share the URL-pull rate-limit bucket).
  - Specs-list (Epic 07) row-action Delete: after `deleteSpecAction({ specId })` returns success, call `showToast(TOASTS.specDeleted)` BEFORE `router.refresh()`. No rate-limit branch (`deleteSpecAction` has no rate-limit).
...
21. **Toast wiring on Spec Detail header buttons** (per Scope §"Toast wiring on existing surfaces" / per Epic 05 results §"Risks → Epic 08"): `spec-detail-header.tsx` `onRepull` calls `showToast(TOASTS.rePullComplete)` on success and `showToast(formatQuotaToast(error))` on `rate_limited`; `onReanalyze` calls `showToast(TOASTS.reanalyzeStarted)` on success; `spec-detail-view.tsx` `FailedPanel.onRetry` also calls `showToast(TOASTS.reanalyzeStarted)`. Epic 06's stale-card Re-analyze button calls `showToast(TOASTS.reanalyzeStarted)`. Epic 07's specs-list row-action menu wires Re-analyze (`TOASTS.reanalyzeStarted`), Re-pull (`TOASTS.rePullComplete` on success / `formatQuotaToast(error)` on `rate_limited`), and Delete (`TOASTS.specDeleted` on success). Seven wiring points total. Vitest tests assert the toast invocation per click handler.
