...
- `(app)/specs/[specId]/page.tsx` — Spec Detail screen with `getRequiredSession()` workspace-scoping. **Replaces** Epic 03's placeholder at the same path (which currently renders spec name + source URL + status + endpoint count). The placeholder's auth/workspace check pattern (`getRequiredSession()` → `prisma.spec.findFirst({ where: { id, workspaceId } })` → `notFound()` on miss) is the established convention — keep it, expand the rendered content. **Note (per Epic 02 results)**: `(app)/layout.tsx` is already wrapped in `<TooltipProvider>` from Epic 02, so the "disabled with tooltip" Apply/Reject buttons (and any other Tooltip primitives in this screen) work without further wrapper setup.
- **Prisma type imports** (per Epic 04 results §6): `Finding`, `Spec`, `SpecVersion` type aliases live at `@/generated/prisma/client` (NOT `@/generated/prisma/models`, despite CLAUDE.md). The CLAUDE.md note will be updated; until then, follow Epic 04's pattern (e.g. `src/lib/analysis/quality-score.ts`).
- **`(app)/specs/error.tsx` and `not-found.tsx` already exist** (Epic 03 shipped minimal versions scoped to the `/specs` segment). Epic 05 inherits them — no need to create. Epic 08 polishes per `prd-decisions.md` Cards conventions.
- **Re-pull button visibility logic**: render the "Re-pull from URL" button only when `Spec.sourceType === 'url' AND Spec.wasAuthedPull === false` (per Epic 03's `wasAuthedPull` boolean — auth-headers aren't persisted, so authed pulls aren't re-pullable). Sample-sourced specs (`sourceType === 'sample'`) also hide the button. Epic 03's `repullSpecAction` enforces this at the action level (returns `{ kind: 'not_repullable' }`); Epic 05 hides the UI for parity.
- **Sidebar hydration warning** is pre-existing (Epic 03 results / Epic 04 browser verification reconfirmed) and not Epic 05's responsibility. Tooltip primitives in this screen (disabled Apply/Reject buttons, endpoint-list rows, truncated URLs) work despite the warning. If the issue becomes user-facing, hand off to a separate `/patch` or Epic 08's polish pass — do not investigate inside Epic 05.
...
1. `/specs/[specId]` for a valid spec in the user's workspace renders the header with name, quality-score badge (colour thresholds per `prd-decisions.md` §"Color Palette" — ≥80 emerald, 60–79 amber, <60 red; if `qualityScore IS NULL` — e.g. before the first successful analysis — render a neutral placeholder badge `—` in zinc; after a `failed` re-analysis on a previously-completed spec, the prior numeric score is retained per cross-epic Q3 — `runAnalysis` only writes `qualityScore` inside its success transaction, so the field accurately reflects the last completed analysis and the `failed` status pill alongside signals the retry need), status pill (colours per `prd-decisions.md` §"Components" Status-Pills mapping), source URL, last-analyzed timestamp.
2. While `analysisStatus` is `pending` or `analyzing`, the right pane shows a spinner and the client polls every 3 s; auto-stops on `completed` or `failed`. When status flips to `completed`, the findings render without a manual refresh.
...
13. `analysisStatus = failed` renders an error card with the `analysisError` message and a primary "Retry analysis" button (violet per `prd-decisions.md` §"Components" Buttons); clicking it triggers `reanalyzeSpecAction` from Epic 04. **`analysisError` formatting** is centralised in Epic 08's `formatAnalysisError(raw: string)` helper at `src/lib/format-analysis-error.ts` (per cross-epic Q6). It returns `{ headline, details?, budgetShape? }`. The failed-card renders `headline` as the heading plus, if `details` is present, a collapsible `<details>` with `<pre>` showing the parsed JSON details. For zod-error JSON inputs, the helper produces a headline of the form `` `<path.join('.')>: <message>` `` (e.g. "`findings[9].rationale`: Invalid input: expected string, received undefined"; per cross-epic Q1) so the user knows WHICH finding failed. **No client-side optimistic state required**: `reanalyzeSpecAction` writes `analysisStatus = 'analyzing'` synchronously before returning (per Epic 04 results §"Resolved Q3" / commit `50b4b1c`); the existing 3 s poller picks up the new status on its next tick. Consumers may also call `router.refresh()` in the click handler to skip the next poll wait.
...
- (resolved per cross-epic Q1, 2026-05-02) `analysisError` headline format on the failed card includes the zod issue path: `` `<path.join('.')>: <message>` ``. Implemented inside Epic 08's `formatAnalysisError` helper (per Q6).
