...
- `(app)/specs/[specId]/page.tsx` — Spec Detail screen with `getRequiredSession()` workspace-scoping. **Replaces** Epic 03's placeholder at the same path (which currently renders just spec name + status + endpoint count). The placeholder's auth/workspace check pattern (`getRequiredSession()` → `prisma.spec.findFirst({ where: { id, workspaceId } })` → `notFound()` on miss) is the established convention — keep it, expand the rendered content. **Note (per Epic 02 results)**: `(app)/layout.tsx` is already wrapped in `<TooltipProvider>` from Epic 02, so the "disabled with tooltip" Apply/Reject buttons (and any other Tooltip primitives in this screen) work without further wrapper setup.
...
- **Sidebar hydration warning** (pre-existing, surfaced during Epic 03 browser verification): `SidebarMenuButton` with the `tooltip` prop emits a `data-state` attribute that mismatches between SSR and client. Doesn't break functionality but appears as a console error. This screen adds heavy Tooltip use (disabled Apply/Reject buttons + endpoint-list rows + truncated URLs); consider investigating the root cause before adding more Tooltips. Likely fix: defer Tooltip mount until after hydration via `useEffect`-gated client component, or pin the Tooltip's controlled `open` state to `false` on SSR.
