...
- `reanalyzeSpecAction({ specId })` server action — manual re-trigger from Spec Detail. Same path as the auto-trigger. **Workspace check**: must call `getRequiredSession()` first (per Epic 02 convention) and verify `Spec.workspaceId === session.workspaceId` — return 404 on cross-workspace access. Then fire-and-forget POST to `/api/internal/analyze` with the `x-internal-secret` header (same shape as Epic 03's `addSpecFromUrlAction` step 12). **Note on `budget_exceeded` UX**: because the trigger is fire-and-forget, the user does NOT receive `{ kind: 'budget_exceeded' }` synchronously from the action. The budget rejection is surfaced via `Spec.analysisStatus = 'failed'` + `Spec.analysisError` (set by `runAnalysis` when budget check fails — see dollar-budget check above), which Epic 05's failed-card renders. If a future v0.2 wants synchronous budget feedback, change the trigger to await the route response and have the consumer call `showToast(formatQuotaToast(error))` from `@/lib/toasts` (Epic 08).
