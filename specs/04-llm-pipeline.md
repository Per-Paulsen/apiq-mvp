...
- **Spike-to-runtime file mapping** (per Epic 00 results §"What Epic 04 should do"):
  ...
  - `scripts/spike/stringify-spec.ts` → `src/lib/analysis/stringify-spec.ts` (`cycleStripSpec` + `stringifySpecForPrompt`). Used by both Epic 04 (prompt build) and Epic 06 (patch validator); they MUST share the helper so they observe the same tree. **Already ported by Epic 03** (verbatim from the spike — both functions exported); Epic 04 just imports.
  ...
  - **No spike source** → `src/lib/analysis/quality-score.ts` (new file owned by Epic 04). Exports `computeQualityScore(findings: Finding[]): number` per the formula below. Imported by both `runAnalysis` (this epic) and Epic 06's `applyFindingAction` / `rejectFindingAction` / `undoApplyAction` / `undoRejectAction` for transactional score recomputation.
...
- Implement `src/lib/analysis/runAnalysis.ts`:
  ...
  - **dollar-budget check:** `SELECT SUM(costUSD) FROM LLMCall WHERE workspaceId = X AND createdAt > NOW() - INTERVAL '24 hours'`. If sum ≥ `$10.00`, **set `Spec.analysisStatus = 'failed'` + `Spec.analysisError = 'Daily LLM budget reached ($<spent> / $10.00) — resets at <retryAt>'`** so Epic 05's failed-card surfaces the error to the user (the auto-trigger from Epic 03 is fire-and-forget; without this status update the spec stays in `'pending'` indefinitely). Then reject with `{ kind: 'budget_exceeded', spent: <sum>, limit: 10.00, retryAt: <oldest call's createdAt + 24h> }`. The $10/24h figure is the Q4 confirmed decision per `specs/ind-epic-review.md`; chosen because reasonable engineer usage (5-7 medium specs/day) stays well below it while a misuse vector (Stripe-class loop) is capped at ~$10 instead of ~$90.
...
- Implement `src/app/api/internal/analyze/route.ts`:
  ...
  - **internal-only**: checks a shared secret header `x-internal-secret` against `process.env.INTERNAL_API_SECRET` to prevent external invocation. Kept for manual debugging via `curl` and as a future entry point if a queue worker is added in v0.2.
- **Auto-trigger via direct function call** (per Epic 03 results recommendation #1 — confirmed by user): Epic 03's `addSpecFromUrlAction` and `repullSpecAction` currently use `fetch('http://localhost:3000/api/internal/analyze', …)` as a fire-and-forget self-fetch (with the localhost-hardcode known issue). When this epic ships, **replace the self-fetch in Epic 03's `triggerAnalyzeFireAndForget` helper with a direct call**: `void runAnalysis(specId).catch((err) => console.error('runAnalysis failed:', err))`. This eliminates the URL-config issue, the secret-header roundtrip, and the localhost-hardcode in one move. Both endpoints run server-side in the same Node process — the fetch was unnecessary. The `/api/internal/analyze` route stays for manual/debug callers; it just isn't the auto-trigger anymore. `reanalyzeSpecAction` below uses the same direct-call pattern.
- **Wire deferred Finding-invalidation in Epic 03's `repullSpecAction`** (per Epic 03 results §"Risks for future epics" — Epic 03 left a `// TODO Epic 04` comment because the `Finding` model didn't exist yet). When this epic ships the model, add inside Epic 03's re-pull transaction: `await tx.finding.updateMany({ where: { specId, status: 'open' }, data: { status: 'outdated' } });`. Prisma's `@updatedAt` auto-handles the `updatedAt` bump on `updateMany` — don't set it manually. Removes the TODO. Epic 03's AC #14 ("sets old `open` Findings to `outdated`") becomes assertable — extend Epic 03's existing `actions.test.ts` with a test that seeds a `Finding` row, runs `repullSpecAction`, and asserts the row's `status === 'outdated'`.
- `reanalyzeSpecAction({ specId })` server action — manual re-trigger from Spec Detail. **File location**: lives in `src/app/(app)/specs/actions.ts` alongside Epic 03's existing actions (`addSpecFromUrlAction`, `repullSpecAction`, `loadSampleSpecAction`, `deleteSpecAction`) — same workspace-scoped server-action conventions, same import path for consumers (Epic 05 imports `reanalyzeSpecAction` from `@/app/(app)/specs/actions`). **Workspace check**: must call `getRequiredSession()` first (per Epic 02 convention) and verify `Spec.workspaceId === session.workspaceId` — return 404 on cross-workspace access. Then fire-and-forget direct call: `void runAnalysis(specId).catch((err) => console.error('runAnalysis failed:', err))`. Button-triggered (not form-triggered), so the consumer (Spec Detail's "Retry analysis" button per Epic 05 AC #13) calls it via `useTransition` or an async click handler — no `useActionState` / FormData adapter needed (per Epic 03 results convention: forms use the object-args + form-action.ts adapter pattern; buttons call object-args actions directly). **Note on `budget_exceeded` UX**: because the trigger is fire-and-forget, the user does NOT receive `{ kind: 'budget_exceeded' }` synchronously from the action. The budget rejection is surfaced via `Spec.analysisStatus = 'failed'` + `Spec.analysisError` (set by `runAnalysis` when budget check fails — see dollar-budget check above), which Epic 05's failed-card renders. If a future v0.2 wants synchronous budget feedback, change the trigger to `await runAnalysis(specId)` and have the consumer call `showToast(formatQuotaToast(error))` from `@/lib/toasts` (Epic 08).
...
1. Prisma migration `add_finding_and_llm_call` creates `Finding` + `LLMCall` with the schema above. Indexed on `(specId, status)` for filtering and on `(workspaceId, createdAt)` for cost audits. Implementation note (per Epic 02 results): `npx prisma migrate dev` in Prisma 7.x does NOT auto-run `npx prisma generate` — run it explicitly so `@/generated/prisma/models` picks up `Finding` and `LLMCall` (imported as `import type { Finding, LLMCall } from '@/generated/prisma/models'`).
...
- Internal-API-secret rotation: a single env var is fine for v0.1. Multi-secret rotation is v0.2+ infra. **Pre-launch checklist (per Epic 02 results):** the current `.env` `INTERNAL_API_SECRET` is a dev placeholder string; replace with `openssl rand -base64 32` output before any deploy. The route returns 403 for missing/wrong header so a placeholder secret in prod would silently break analysis triggers.
