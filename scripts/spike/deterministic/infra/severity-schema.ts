...
// =============================================================================
// 11b. apiq-meta YAML-block validator (Welle Arch+ A2b)
// =============================================================================

/**
 * Map of YAML kebab-case keys -> camelCase RuleMetadata keys. Only keys that
 * differ are listed; identical names (`severity`, `lenses`, `sources`,
 * `stakeholders`, `iso25010`, `priority`, `direction`) pass through.
 */
const YAML_KEY_TO_CAMEL: Record<string, string> = {
  'pattern-id': 'patternId',
  'lifecycle-phase': 'lifecyclePhase',
  'defect-class': 'defectClass',
  'codegen-targets': 'codegenTargets',
  'detection-precision': 'detectionPrecision',
  'auto-fix-safe': 'autoFixSafe',
  'regulatory-mapping': 'regulatoryMapping',
  'cost-impact': 'costImpact',
  'mttr-impact': 'mttrImpact',
  'agent-readiness-impact': 'agentReadinessImpact',
};

/**
 * Convert kebab-case YAML keys at the top level of an apiq-meta block into the
 * camelCase shape expected by `RuleMetadataSchema`. Nested objects (e.g.
 * `regulatory-mapping`) already use camelCase or all-lowercase keys
 * (nist/asvs/cis/gdpr/soc2) so no recursive walk is needed.
 */
function kebabToCamelTopLevel(
  block: Record<string, unknown>,
): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const [k, v] of Object.entries(block)) {
    const camel = YAML_KEY_TO_CAMEL[k] ?? k;
    out[camel] = v;
  }
  return out;
}
...
/**
 * Validate an apiq-meta YAML block (kebab-case shape) against
 * `RuleMetadataSchema` (camelCase). Returns the parsed `RuleMetadata` on
 * success, or `{ errors: [...] }` on failure with one string per zod-issue.
 *
 * Used by `spectral-runner.buildRulesAccFromYaml` at YAML-load time to surface
 * malformed apiq-meta blocks as warnings without aborting the runner (graceful
 * degradation — invalid blocks still pass through to consumers, but the warn
 * makes the drift visible in CI logs).
 */
export function validateApiqMetaYamlBlock(
  block: unknown,
): RuleMetadata | { errors: string[] } {
  if (!block || typeof block !== 'object' || Array.isArray(block)) {
    return { errors: ['apiq-meta block is not an object'] };
  }
  const camel = kebabToCamelTopLevel(block as Record<string, unknown>);
  const result = RuleMetadataSchema.safeParse(camel);
