...
> **Status:** v0.1 done (Epics 00–08). **Epic 09 Big-Spec Architecture Spike: PAUSED at Phase 0 entry (2026-05-05). Critical review found 13 blind spots in Draft 0.11. Decision: Phase 0 = Eval-Framework (~1 day), Phase A = Deterministic Layer (~2-4 days), Phase B = finale (C-i)-Messung mit multi-run, Phase C = Lock. See `specs/big-spec-architecture-spike-critical-review.md` for full record.** Conditional Spike-Epics 10–13 not yet spec'd.
...
>
> **Next (resuming Epic 09):** read `specs/big-spec-architecture-spike-critical-review.md` first (full critical review + decisions + Phase 0 → A → B → C plan). Then `LAUNCH-PROGRESS.md` "2026-05-05" Setup-Actions-Log entry. Architectural insight: 50-65% PD / 40-55% Stripe der LLM-Findings sind deterministisch findbar → apiq-Architektur ist Hybrid (Deterministic Layer + LLM Layer), nicht LLM-First. Phase 0 = Eval-Framework im `scripts/spike/eval/`-Harness; Phase A = Deterministic Layer; Phase B = finale (C-i)-Messung mit multi-run statistical signal; Phase C = Lock.
>
