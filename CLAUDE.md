...
> **Status:** v0.1 done (Epics 00–08). **Epic 09 Stage A — Welle 0+A+B + W1+W2+W3+W4+Q + Welle M done (2026-05-07).** `runDeterministicLayer` jetzt full: Spectral (110 active rules across 4 yamls) + 16 walkers + 15 module-classes (`modules/index.ts`). 16 von 17 module-classes wired; `spec-diff` bleibt orphan (2-Spec lifecycle). Alle 5 custom spectral-functions aktiv. **Welle M (Mining-Maximierung):** 122 Round-3-candidates (51 books + 42 postmortems + 11 corpus-derived + 18 reaudit-orphans) integriert in master `rules-brainstorm.md` (1797→2075 Zeilen). 100% Source-Mapping-Comments auf 110 YAML-rules + 5 custom-functions + 15 module-class headers. **871 patterns** committed `scripts/spike/data/patterns.json` (Round-1+2+3). **Pattern-Knowledge-Index** 678 entries embedding-based via `findRelatedPatterns` API (gitignored 26MB cache, reproducible). **API-Corpus-Library** 10 statistics über 521 healthy public OpenAPI specs (gitignored 253MB, reproducible via `download-corpus.mjs`). 8 alte Mining-Files konsolidiert zu Stubs (≤40 Zeilen) — `mining-round2-meta.md` bleibt eigenständig. **833 tests pass / 41 files / 2 skipped / 0 fail (+27 neue Tests vs Welle-Q-baseline)**. **Top-Round-3-Findings:** RFC-7807 0% adoption + Sunset/Deprecation 0% adoption + 22.2% unsecured-write-ops in healthy public APIs + OAuth2-Implicit-deprecation RFC 9700 BCP-240 (Jan 2025) + GitHub `x-brownout-schedule` als Lens-3 positive-marker. **Stage-A-Coverage (unverändert vs Welle Q — M war Mining-Maximierung, nicht Coverage-lifter):** stripe-full **62.1%** | pagerduty-full **69.6%** Embedding | dnd5eapi **85.7%** Embedding (= Prediction) | github-rest **64.5%** Embedding. **Round-4-Decision (D14):** Trigger erfüllt aber conditional-on-user — Mining ehrlich maxed-out für Stage-A. Welle F nutzt patterns.json + pattern-index als Substrate für 110-rule-metadata-promotion + autoFixSafe-tagging + Schema-Erweiterungen.
> **Done:** Epic 00 (Research Spike) → Epic 08 (Export + Polish). 298 tests pass v0.1 main, **833 tests pass on v1-launch (Stage A post-Welle-M)**.
...
>
> **Next (resuming Epic 09):** Welle Q + W1-W4 + Welle M done. **Welle F als nächste Welle.** Verbindlicher Plan v2 in `specs/big-spec-architecture-spike-stage-a-restwork-plan.md` — Putzen-First-Reihenfolge: ~~Welle Q~~ ✓ → ~~Welle M~~ ✓ → **Welle F** (Framework-Optimization F1-F10 inkl. 110-rule metadata-promotion + autoFixSafe + ai-agent stakeholder-enum + direction-modifier structured + apiq-meta YAML-block) → **Welle C/D** (P2/P3 Spectral) → **Welle D2** (P4+P5) → **Welle E** (Putz-Niveau-Benchmark) → **Welle T** (Test-Coverage all-specs + Snapshot + CI) → **Welle Doc** → **Welle Arch** (flat → classifiers/aggregators/modules/rules) → **Welle R** (R1+R2 Multi-Model-cross-validated) → **Welle V** (4-way Cross-Linter, **erst hier!**) → **Phase B** (N=3×4 Specs). **Kritisch:** Cross-Linter-Vergleich gehört NACH Putzen. **Memory-Regeln:** niemals Fixes verschieben (`memory/feedback_never_defer_fixes.md`); niemals Time/Cost-Estimates (`memory/feedback_no_engineering_time_estimates.md`) — Maximalismus-Setup. Execution via **`/spec_ind w-f-framework-optimization` + `/dev`**. Plan-Doc §21 hat Welle-Status-Tracker. **Resume-Trigger neue Session:** "welle f starten" oder "weiter mit restwork-plan v2 — welle f". Welle-M-Results: `specs/E09-w-m-mining-optimization-results.md`. Welle-Q-Results: `specs/E09-w-q-code-quality-cleanup-results.md`. patterns.json single-source: `scripts/spike/data/patterns.json`. Pattern-Index reproducible via `npx tsx scripts/spike/eval/build-pattern-index.ts` (needs OPENAI_API_KEY).
>
