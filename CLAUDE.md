...
>
> **Next (resuming Epic 09):** Welle Q + W1-W4 + Welle M + Welle F + Welle C + Welle D + Welle Arch+ + Welle D2 + Welle I done. **Welle E als nächste Welle (T24 Putz-Niveau-Benchmark gegen 28 Springer-Delphi-Rules; Welle I substrate jetzt verfügbar — E0+E1 konsumieren `inventory.json` + `coverage.json`).** Verbindlicher Plan v2 in `specs/big-spec-architecture-spike-stage-a-restwork-plan.md` — Putzen-First-Reihenfolge: ~~Welle Q~~ ✓ → ~~Welle M~~ ✓ → ~~Welle F~~ ✓ → ~~Welle C~~ ✓ → ~~Welle D~~ ✓ → ~~Welle Arch+~~ ✓ (vorgezogen 2026-05-09) → ~~Welle D2~~ ✓ (P4+P5, 11 echt-neue rules — 15 von 26 Plan-Doc-§8-Patterns waren bereits in vorigen Wellen via Pre-D2-Audit) → ~~Welle I~~ ✓ (Inventory + Capability-Map; auto-generated single-source-of-truth + CI-gate) → **Welle E** (Putz-Niveau-Benchmark; T-Stripe-Perf bereits in Arch+ done) → **Welle T** (Test-Coverage all-specs + Snapshot + CI; parallel zu E) → **Welle Doc** → **Welle R** (R1+R2 Multi-Model-cross-validated) → **Welle V** (4-way Cross-Linter, **erst hier!**) → **Phase B** (N=3×4 Specs). **Kritisch:** Cross-Linter-Vergleich gehört NACH Putzen. **Memory-Regeln:** niemals Fixes verschieben (`memory/feedback_never_defer_fixes.md`); niemals Time/Cost-Estimates (`memory/feedback_no_engineering_time_estimates.md`); Plan-Doc-Welle-Sections sind source-of-truth (`memory/feedback_plan_doc_is_source_of_truth.md`); INVENTORY/COVERAGE-files vor manuellem Greppen konsultieren (`memory/feedback_consult_inventory_first.md`) — Maximalismus-Setup. Execution via direct-Spec + `/dev`. Plan-Doc §21 hat Welle-Status-Tracker. **Resume-Trigger neue Session:** "welle e starten" oder "weiter mit restwork-plan v2 — welle e". Welle-I-Results: `specs/E09-w-i-inventory-capability-map-results.md`. Welle-D2-Results: `specs/E09-w-d2-niche-vendor-results.md`. Welle-Arch+-Results: `specs/E09-w-arch-architecture-cleanup-results.md`. Welle-D-Results: `specs/E09-w-d-p3-trail-results.md`. Welle-C-Results: `specs/E09-w-c-p2-spectral-rules-results.md`. Welle-F-Results: `specs/E09-w-f-framework-optimization-results.md`. Welle-M-Results: `specs/E09-w-m-mining-optimization-results.md`. patterns.json single-source: `scripts/spike/data/patterns.json` (validated via PatternSchema). Pattern-Index reproducible via `npx tsx scripts/spike/eval/build-pattern-index.ts` (needs OPENAI_API_KEY). **Layered structure post-Arch+:** `scripts/spike/deterministic/{classifiers,aggregators,modules,rules,spectral-functions,iana,infra,index.ts}`. **Inventory artifacts (post-Welle-I):** `scripts/spike/eval/{INVENTORY.md, COVERAGE.md, CROSS-REFERENCES.md, DRIFT-REPORT.md, TEST-COVERAGE.md, API-SURFACE.md}` als single-source-of-truth.
>
...
- **Pre-Welle-Audit (post-Welle-I 2026-05-10):** consult `scripts/spike/eval/INVENTORY.md` + `COVERAGE.md` + `CROSS-REFERENCES.md` + `DRIFT-REPORT.md` + `TEST-COVERAGE.md` + `API-SURFACE.md` BEFORE manual greppen. Files outdated? Run `npm run inventory-all` (fast) or `npm run build-coverage` (~45min) and commit. CI-gate `inventory-drift.yml` enforces freshness on every PR. Memory: `feedback_consult_inventory_first.md`.
