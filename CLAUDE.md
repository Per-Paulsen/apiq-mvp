...
> **Status:** v0.1 done (Epics 00–08). **Epic 09 Stage A — Welle 0+A+B + W1+W2+W3+W4+Q (inkl. Q6/Q7/Q8 follow-ups) done (2026-05-06).** `runDeterministicLayer` jetzt full: Spectral (110 active rules across 4 yamls) + 16 walkers + 15 module-classes (`modules/index.ts`). 16 von 17 module-classes wired; `spec-diff` bleibt orphan (2-Spec lifecycle). Alle 5 custom spectral-functions aktiv. Welle Q lieferte: codegen-output-aggregation (9.834→1 findings), OPENAI_API_KEY env-loading fix (bug! SPIKE_DIR-path war 1 `..` zu viel — Embedding-scorer war silent skipped seit forever), module-class Layer-Tag, 4 Integration-Tests, STAGE-A-RESULTS.md section-aware merge bei --specs subset, `affectedEndpoints` cap bei aggregated findings. 806 tests pass / 38 files / 2 skipped / 0 fail. **Stage-A-Coverage (4-Spec re-run mit Embedding):** stripe-full **62.1%** Jaccard / **62.1%** Embedding | pagerduty-full **30.4%** Jaccard / **69.6%** Embedding (+39.2pp) | dnd5eapi **35.7%** Jaccard / **85.7%** Embedding (= Prediction) | github-rest **25.8%** Jaccard / **64.5%** Embedding (+38.7pp). **Strategischer Befund (revidiert post-Embedding):** Vocabulary-Drift war der dominant Gap. Delta vs Predicted shrinkt von -50/-61pp (Jaccard) auf nur -20/-22pp (Embedding-best). Stage-A ist deutlich näher an Predictions als die Jaccard-only-Numbers vorher zeigten. Welle C/D/E könnte weitere ~5-10pp lift bringen (nicht 0-3pp wie pre-Embedding-framing). Phase B bleibt eigentlicher Spike-Lock-Test, Pre-Conditions Q1+Q6+Q7+Q8 alle resolved. **Pattern-Mining declared done at 10 Lenses** (Round-2 Phase F diminishing-yield) — Welle M Mining-Round-3 entscheidet ob das Maximum wirklich erreicht ist.
...
>
> **Next (resuming Epic 09):** Welle Q + W1-W4 done. Welle M Spec ready, `/dev` pending. **Verbindlicher Plan v2 (Maximalismus + /spec_ind/dev-Workflow) in `specs/big-spec-architecture-spike-stage-a-restwork-plan.md`** — Putzen-First-Reihenfolge: ~~Welle Q~~ ✓ → **Welle M** (Mining-Round-3 + M2 Corpus + M3 Konsolidierung + M4 Code-Comments + M5 Pattern-Index) → **Welle F** (Framework-Optimization F1-F10 inkl. 110-rule metadata-promotion) → **Welle C/D** (P2/P3 Spectral) → **Welle D2** (P4+P5) → **Welle E** (Putz-Niveau-Benchmark) → **Welle T** (Test-Coverage all-specs + Snapshot + CI) → **Welle Doc** → **Welle Arch** (flat → classifiers/aggregators/modules/rules) → **Welle R** (R1+R2 Multi-Model-cross-validated) → **Welle V** (4-way Cross-Linter, **erst hier!**) → **Phase B** (N=3×4 Specs). **Kritisch:** Cross-Linter-Vergleich gehört NACH Putzen (`critical-review.md:528, :585`). **Plus Memory-Regeln:** niemals Fixes verschieben (`memory/feedback_never_defer_fixes.md`); niemals Time/Cost-Estimates (`memory/feedback_no_engineering_time_estimates.md`) — Maximalismus-Setup. Execution via **`/spec_ind w-{welle}-{slug}` + `/dev`**, Plan-Doc §21 hat Welle-Status-Tracker. **Resume-Trigger neue Session:** "welle m /dev starten" — Spec liegt unter `specs/E09-w-m-mining-optimization.md`, Brainstorming + Decisions in `*-brainstorming.md` §10/§12. Audit-Reports: `specs/big-spec-architecture-spike-stage-a-{implementation,claim}-audit.md`. Welle-Q-Results: `specs/E09-w-q-code-quality-cleanup-results.md`. Run-Output: `specs/big-spec-runs/eval/STAGE-A-RESULTS.md` (post-Welle-Q mit Embedding-Coverage).
>
