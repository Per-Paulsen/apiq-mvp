...
> **Status:** v0.1 done (Epics 00–08). **Epic 09 Big-Spec Architecture Spike: Phase 0 (Eval-Framework) DONE (2026-05-05). Phase A (Deterministic Layer, ~2-4 days) is next. 13 blind spots from critical review documented; eval-pipeline operational with structured Stripe reference (29 findings classified) + locked baseline snapshot. See `specs/big-spec-architecture-spike-critical-review.md` for full record.** Conditional Spike-Epics 10–13 not yet spec'd.
...
>
> **Next (resuming Epic 09 — start Phase A):** read `specs/big-spec-architecture-spike-critical-review.md` (critical review + Phase 0/A/B/C plan) and `LAUNCH-PROGRESS.md` "2026-05-05" Setup-Actions-Log entries. Phase 0 eval-pipeline lives at `scripts/spike/eval/` (runner + scorers + snapshot + comparison + score-run). Locked baseline at `scripts/spike/eval/snapshots/c-i-baseline-stripe.json` reproduces Stage-3 numbers (1423 findings, 37.9% coverage, 28.6% knowledge-backed-coverage — empirically validates v6-prompt defizit). Phase A starts: scripts/spike/eval/ stays stable as eval-harness; Stage 4 = Deterministic Layer in scripts/spike/deterministic/ that pre-passes specs and emits findings the LLM does not need to re-find. Then Phase B (finale C-i Messung mit Pre-Pass + v6 + Cache + multi-run) → Phase C (Lock).
>
