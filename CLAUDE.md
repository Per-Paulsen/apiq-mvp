...
>
> **Next (resuming Epic 09):** Welle Q + W1-W4 done. **Verbindlicher Plan v2 (Maximalismus + /spec_ind/dev-Workflow) in `specs/big-spec-architecture-spike-stage-a-restwork-plan.md`** — Putzen-First-Reihenfolge: ~~Welle Q~~ ✓ → **Welle M** (Mining-Round-3 curated + M2 Corpus-Mining VERPFLICHTEND + Konsolidierung + Code-Comments) → **Welle F** (Framework-Optimization F1-F10 inkl. ai-agent-Stakeholder fehlt + 2 Defect-Classes fehlen + 110-rule metadata-promotion) → **Welle C/D** (P2/P3 Spectral) → **Welle D2** (P4+P5 Niche/Vendor) → **Welle E** (Putz-Niveau-Benchmark T24) → **Welle T** (Test-Coverage all-specs + Snapshot + CI) → **Welle Doc** (dev-README + Architecture-Diagram + Contributor-Guide) → **Welle Arch** (flat → classifiers/aggregators/modules/rules) → **Welle R** (R1+R2 verpflichtend Multi-Model-cross-validated) → **Welle V** (4-way Cross-Linter Vacuum+Redocly+IBM+Spectral-OWASP, **erst hier!**) → **Phase B** (N=3×4 Specs für statistical confidence). Total ~55-85h Stage-A + 5-8d Phase B + ~$60-80 LLM (Welle Q done -3-5h). **Kritisch:** Cross-Linter-Vergleich gehört NACH Putzen, nicht davor (`critical-review.md:528, :585`). **Plus Memory-Regel: niemals Fixes auf später verschieben** (`memory/feedback_never_defer_fixes.md`) — wenn ein Issue konkret + fixable ist, sofort fixen. Execution via **`/spec_ind w-{welle}-{slug}` + `/dev`**, Plan-Doc §21 hat Welle-Status-Tracker. Resume-Trigger: "welle M starten" — `/spec_ind` mit Brainstorming weil Source-Selektion + Round-N-Decision-Points Discussion brauchen. Audit-Reports: `specs/big-spec-architecture-spike-stage-a-{implementation,claim}-audit.md`. Welle-Q-Results: `specs/E09-w-q-code-quality-cleanup-results.md`. Run-Output: `specs/big-spec-runs/eval/STAGE-A-RESULTS.md` (post-Welle-Q regeneriert mit Embedding-Coverage).
>
